...
XNU_OSKEXT_POLICY_PATCH=$SCRIPT_DIR/../patches/xnu-4570.41.2/0005-add-fixed-kext-set-boot-mode.patch
...
if ! grep -q -- '-no-kext-autounload' "$XNU_OSKEXT_SOURCE"; then
    patch -d "$SOURCE_ROOT/xnu" -p1 < "$XNU_OSKEXT_POLICY_PATCH"
fi
grep -q -- '-no-kext-autounload' "$XNU_OSKEXT_SOURCE"
